...
class PrintHelper {
    ...
    template<typename T>
    void printArray(T a, int size, uint8_t nybbles);

    // Print hex numbers with 0 padding up to the specified number of nybbles.
    template<typename T>
    void printPadded(T w, uint8_t nybbles);

    // Handle format enums for printing single hex numbers.
    template<typename T>
    void handleX(uint8_t nybbles, va_list *src);
    
    // Handle format enums for printing hex arrays.
    template<typename T>
    void handleArray(uint8_t nybbles, va_list *src);
    ...
    public:
        PrintHelper(const Print &print) : mPrint(print) {}
